...
The UI ships with 24 languages (YAML files under `lib/I18n/translations/`):
...
Run generator commands from the `crosspoint-reader/` directory (or prefix with `cd crosspoint-reader` from the monorepo root).
...
```bash
uv run python scripts/gen_i18n.py lib/I18n/translations lib/I18n/
```
...
Language settings are stored with the rest of the device settings in:
```
/.crosspoint/settings.json
```

The `language` value is persisted as the stable language code string, such as `EN` or `DE`.
...
2. Run `uv run python scripts/gen_i18n.py lib/I18n/translations lib/I18n/`
...
5. Run `uv run python scripts/gen_i18n.py lib/I18n/translations lib/I18n/` to verify
