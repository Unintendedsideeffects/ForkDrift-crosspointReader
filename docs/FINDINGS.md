...
## 2026-08-28T14:40Z — The 32 KB inflate window costs ~24 KB of contiguity at book open, but steady-state measurement hit a 33 KB noise floor
- **Found by**: claude — throwaway experiment build (never committed), following the multi-agent architecture review in `plans/heap-2026-08-28/architecture/`
- **Where**: `lib/InflateReader/InflateReader.cpp:9` (`INFLATE_DICT_SIZE`), `lib/Epub/Epub.cpp` (both `ensureSharedWindow()` sites)
- **What**: reading a warm-cached book performs **zero** inflate operations (measured: no ZIP activity across page turns; a cache hit reads `book.bin`, `sections/` and the CSS cache, never the archive). So the window can be suppressed on the warm path to isolate its cost without needing a STORED book at all — no SD access required, and a better-controlled comparison than a `zip -0` twin, which also differs in file size, entry layout and `book.bin` contents. Measured at book open, matched walks, clean reboots: window reserved gave **44,136 free / 20,468 largest**; window suppressed gave **76,380 free / 45,044 largest**. That is +32 KB free and **+24.5 KB largest**, past the 16 KiB threshold set as the falsifier. The window's cost is real.
- **Two results that cut the other way, and matter more than the headline**: (1) with the window suppressed, *steady* contiguity got **worse** (~11,764 vs ~20,468), because an uncached chapter then claimed the window lazily from an already-working heap — precisely what the existing "reserve defensively" comment predicted. Reserving early is correct; the open question is only whether to reserve at all. (2) Three runs of an **identical** configuration (same book, same firmware, window reserved, rebooted between each) produced 45,128 / 78,000 / 45,220 bytes free — a **33 KB spread**. At that noise level a 3-run comparison cannot resolve a 24 KB effect, so no steady-state conclusion should be drawn from small samples on this device.
- **Why not fixed here**: this was measurement, not a change. The experiment gate was reverted and never committed. What it supports — a transactional STORED shadow archive — is a substantial pipeline that should not start until a properly-powered steady-state measurement clears Gate 0.
- **Protocol for anyone repeating this**: reboot between every run; verify the reader actually opened (`enter EpubReader`) and which book was loaded; discard any run logging `Cache not found, building` (an index legitimately claims the window) or whose `t_ms` goes backwards (a silent defrag reboot resets the heap mid-walk and flatters that arm). Harness and all thirteen run CSVs: `plans/heap-2026-08-28/architecture/window-experiment/`.
- **Status**: open — Gate 0 neither passed nor failed; the open-time delta supports the change. **The "33 KB noise floor" in this entry is wrong — see the 2026-08-28T15:30Z entry.** It was a single window-allocation failure, not measurement noise; with the two modes separated the environment repeats to ~2 KB on free heap.

## 2026-08-28T14:45Z — Architecture review: four candidate fixes adjudicated, STORED-shadow selected, framebuffer-loan rejected as a first fix
- **Found by**: cursor (four independent angles) + codex (adjudication) — full reports in `plans/heap-2026-08-28/architecture/`
- **Where**: `plans/heap-2026-08-28/architecture/VERDICT.md` and `cursor-C{1..4}-*.md`
- **What**: the recommendation is to remove the 32 KB process-lifetime pin by building a **validated, disposable STORED shadow archive** beside the derived data, keeping the original EPUB as source of truth, and using a framebuffer loan plus `InflateStream` only as the one-time conversion engine. Routing *all* ZIP inflate through a framebuffer loan was rejected as a first fix: `InflateStream` is not only used by Sleep/TRMNL — `PngDecodeContext` embeds it and it is reached from EPUB cover generation (`lib/Epub/Epub.cpp:790-810`), thumbnails (`844-934`) and Pokemon sprites (`src/util/PokemonSpriteCache.cpp:51-66`) — so the locking obligation is far wider than it first appears. Also established: `FrameBufferLoan` requires the caller to hold `RenderLock` for the loan's lifetime (`lib/GfxRenderer/GfxRenderer.h:274-282`) and `onEnter()` runs outside it (`src/activities/ActivityManager.cpp:196-201`), so loans taken there violate a documented contract rather than merely racing; and `sizeof(PNG)` is 58,912, exceeding both the 52,272-byte framebuffer array and the 48,000-byte loanable scratch, so PNG decode can never borrow.
- **Why not fixed here**: Gate 0 (a controlled largest-block measurement) has not been passed, and the design is a multi-step storage pipeline carrying power-loss, migration and rollback obligations.
- **Status**: open — design agreed, gated on measurement

## 2026-08-28T15:30Z — The inflate window reservation fails intermittently, and that is what looked like measurement noise
- **Found by**: claude — while chasing the apparent 33 KB run-to-run variance recorded at 2026-08-28T14:40Z
- **Where**: `lib/InflateReader/InflateReader.cpp:65-74` (`ensureSharedWindow`), called from `lib/Epub/Epub.cpp` at book open
- **What**: `ensureSharedWindow()` intermittently fails at book open — `[ERR] [INF] Failed to pre-allocate inflate window (32768 bytes)` — because the largest free run at that moment sits right on the 32,768-byte requirement. In the observed failure the largest block afterwards was **31,732 bytes: 1,036 short**. The outcome is therefore **bimodal, not noisy**: when the reservation succeeds free heap reads ~45.2 KB, when it fails it reads ~78 KB because the 32 KB was never taken. Mixing the two modes is what produced the apparent 33 KB spread and the false conclusion that this device cannot be A/B tested. Separating them, five consecutive successful opens gave 45,244 / 47,264 / 45,240 / 45,212 / 45,204 — a ~2 KB spread. Largest block still varies more (10,228-20,468) and needs a median of five or more runs.
- **Why this matters beyond measurement**: a failed reservation is silent to the user but leaves the book unable to inflate, so the next uncached chapter cannot be indexed. That is the same class of failure as the chapter-34 wedge fixed in `3f942e426`, reached by a different route, and it is a strong independent argument for removing the 32 KB requirement altogether rather than tuning around it (see `docs/heap/architecture-verdict.md`).
- **Why not fixed here**: the fix is either to make the reservation reliable (reserve earlier, or reduce the peak that precedes it) or to remove the need for a 32 KB contiguous dictionary. The dictionary size itself is not reducible — DEFLATE correctness requires the full 32 KB window (`TINFL_LZ_DICT_SIZE`).
- **Status**: open — instrumented and measured; see the 2026-08-28T16:15Z entry, which **refutes the boundary hypothesis stated above**

## 2026-08-28T16:15Z — The inflate window normally reserves with 6 KB to spare, so its failures are an anomaly, not routine tightness
- **Found by**: claude — instrumenting `ensureSharedWindow()` to log the largest free block on every attempt
- **Where**: `lib/InflateReader/InflateReader.cpp` (`ensureSharedWindow`, and the lazy claim in `init`)
- **What**: the earlier entry guessed that the 32,768-byte reservation sits *on* the available contiguous run, making success a near coin flip. **That is wrong.** Six instrumented book opens: largest block 42,996 once and then **38,900 on five consecutive runs**, i.e. a margin of 10,228 and then **6,132 bytes, identical five times**. The reservation normally has comfortable headroom and the measurement is strikingly reproducible. The observed failure (largest block 31,732) was therefore not boundary jitter but an occasional consumer taking **more than 6 KB** ahead of the reservation, on some opens and not others.
- **Why this matters**: it changes the shape of the fix. If failures were routine tightness, only removing the 32 KB requirement would help — the expensive STORED-shadow pipeline. If instead a single intermittent consumer eats the margin, finding and deferring it is far cheaper and would make the reservation reliable. The 32 KB *contiguity cost* of holding the window is unaffected either way and remains the argument for the larger change.
- **Next**: identify what allocates >6 KB before the reservation on failing opens. Candidates not yet eliminated: the cover-thumbnail bake (`kMinFreeHeapForCoverThumbBake`, a background task with idle-timer entry), the Terminus fetch task (13,312-byte stack), and a partially-started background server. The instrumentation now logs "short by N" on failure, so the next occurrence is self-explaining; it did not reproduce in six attempts.
- **Status**: open — diagnostic landed, normal margin characterised, cause of the intermittent failure not yet caught

## 2026-08-28T17:30Z — The cover-thumb task allocated a 6 KB stack to discover it could not run
- **Found by**: cursor — static-task-stack analysis (`docs/heap/hunt-2-static-stacks.md`)
- **Where**: `src/activities/reader/EpubReaderActivity.cpp`, `startCoverThumbBakeTask()` / `coverThumbBakeTask()`
- **What**: the affordability gate lived *inside* the task, so a 6,144-byte stack plus its TCB were allocated, the task woke, found it could not proceed, logged "deferred" and exited — freeing the stack again. The gate wants 96,000 free and a 64,000 largest block; the most free heap ever measured on this device is ~78,000 and the working range is 45,000-52,000, so it essentially never passes. Every attempt therefore spent a 6 KB allocation to reach a foregone conclusion. Because `xTaskCreate` takes **two** heap blocks (stack + TCB) and TLSF coalesces the small TCB while leaving the stack as an isolated free run, the freed stack leaves a hole that outlives the task — there is no compaction on this platform. **Fixed**: the check now runs before anything is allocated.
- **Status**: fixed — build- and host-test-verified; the in-task check is retained as belt-and-braces since heap can change between spawn and task start

## 2026-08-28T17:35Z — Probable cause of the intermittent 12,288-byte contiguity loss: the Terminus fetch task stack
- **Found by**: cursor — consumer hunt (`docs/heap/hunt-1-fragmentation-consumer.md`)
- **Where**: `src/features/terminus_sleep/Registration.cpp:351` (`TRMNL_FETCH_TASK_STACK = 12288`), reached from `onBackgroundNetworkReady()` in `BackgroundWifiService`
- **What**: `BackgroundWifiService` connects WiFi and then, **before** starting the web server, calls `onBackgroundNetworkReady()`. That is the ~52 KB Home window in which a 12,288-byte task stack can actually be allocated. If Terminus is configured and a refresh is due it creates the task, waits, then starts the server; the freed stack leaves a 12,288-byte hole. The 15-minute due-gate supplies the intermittency — one boot fetches, the next does not. Arithmetic matches exactly: failing Home `MaxAlloc` 26,612 = succeeding 38,900 − 12,288, with free heap differing by only 76 bytes.
- **Correction to an earlier claim**: this register previously implied `OtaWebCheck` (the other 12,288 constant). That is wrong — it only starts from `POST /api/ota/check`, and it could not succeed in either measured Home state (at ~52 KB the server is not up so the route does not exist; with the server up the largest block is 8,180 < 12,288). Equally, the log used to "rule out" Terminus (`largest=9204`) was the **reading** plateau, not Home — a fetch that already ran and freed at Home produces exactly that later state, and a second attempt then correctly defers.
- **Status**: open — **not confirmed**. The decisive line (`[TRMNL] Network ready; fetching before background server startup` before the Home heartbeat) falls before serial capture can start, because a reset drops the USB/IP attach for ~20 s. Circumstantial support: the observed failure occurred in a session where WiFi was up (`handleClient active, server running`), while six consecutive successes ran with WiFi down, where Terminus can never fetch.

## 2026-08-28T17:40Z — Home is at its composition limit, not a fragmentation limit, and the fix is engineering rather than a product decision
- **Found by**: cursor — adversarial review (`docs/heap/hunt-4-adversarial-review.md`) and server-diet design (`docs/heap/hunt-3-server-diet.md`)
- **Where**: `src/network/server/CrossPointWebServer.cpp:326-411` (51 `server->on()` handlers)
- **What**: post-fix Home is ~11,500 free with an ~8,180 largest block — **71% of free heap is already in one contiguous run**. That is occupancy, not fragmentation; the reading state (57%) is the fragmented one. The investigation applied a fragmentation frame to both, which is why Home never improved. The resident cost is the always-on HTTP stack: ~12,028 B of route registration, ~3,992 B `begin()`, ~5,750 B mDNS, ~4 KB WebSocket/discovery. Each `on()` is three heap blocks (handler object, cloned `Uri`, Arduino `String`) — **not** 2-4 KB of `std::function` each; that CLAUDE.md figure is flash per signature, and all 51 routes share `void()` with `[this]` fitting SBO.
- **What does not work, and why**: lazy `on()` registration drops the first upload, because handler selection happens in `_parseRequest` before the callback and POST bodies stream to the already-matched handler. "Start the server on first connect" loses the first request, because `begin()` only opens the listen socket and cannot adopt an already-accepted client.
- **What does**: a single `RequestHandler` plus a flash dispatch table installed via `addHandler()` — the public escape hatch, already used by `WebDAVHandler` (~12 KB); mDNS started on the first UDP `hello` (~5,750 B), since Android already discovers via UDP 8134 and `.local` is secondary; the WebSocket server started on that same hello (~4 KB); and a lazy font-upload buffer. Estimated Home idle afterwards: **mid-20s KB free with a ~20 KB largest block** — the region where CSS parsing, the AA scratch and the Terminus stack stop being structurally impossible. Each step is independently measurable against the existing `[MEM]` logs.
- **Why this matters**: it means the largest single lever does **not** require changing the `backgroundServerMode` default or removing the feature. It was filed as a product decision; it is an engineering one.
- **Status**: open — designed, not started; P0 is the dispatch table

## 2026-08-28T18:00Z — Shrinking the Terminus fetch stack does not fix the contiguity it costs; un-nesting the SD write does
- **Found by**: cursor (`docs/heap/hunt-5-terminus-stack.md`), prompted by "the fetch stack should be smaller"
- **Where**: `src/features/terminus_sleep/Registration.cpp:351` (`TRMNL_FETCH_TASK_STACK = 12288`), `terminusFetchTask`, `fetchAndPinTrmnlImage`
- **What**: the 12 KB stack is the leading suspect for the intermittent 12,288-byte contiguity loss, so making it smaller looks like the fix. It mostly is not. **Any hole of this size class still breaks the thing we care about** — the inflate window needs 32,768 contiguous bytes, so a 10 KB hole in the middle of the largest run is as fatal as a 12 KB one. Resizing trims the symptom; it does not restore contiguity.
- **What the analysis established**: (a) this device polls **plain HTTP** (`http://192.168.86.25:2300/api/display`), so mbedTLS is not on the path and its 16 KB buffers are heap anyway — but the product still permits `https://trmnl.com`, so a stack sized for the LAN case would crash a cloud user; (b) the peak is **call-chain depth, not a large local**: `esp_http_client_perform` -> lwIP recv -> `imageEventHandler` -> `HalFile::write` -> SdFat/SPI, with the HTTP buffers already on the heap; (c) the 8 KB overflow recorded in the source comment happened on **bgwifi**, which was already holding web-handler frames — so it is weaker evidence for 12,288 than the comment implies.
- **Recommendation**: keep 12,288 until a high-water mark is read from a real image download. Smallest defensible without that number is 10,240, and only for an HTTP-only build; 8,192 is not safe while the SD write stays nested in the HTTP callback. **The change that actually matters is un-nesting**: read into the existing 2 KB heap buffer, then write to SD after `esp_http_client_read` returns, so the two peaks are sequential rather than additive. The fetch could then run synchronously on `bgwifi` in the pre-server window, and the 12,288-byte task — and its hole — would not exist at all.
- **Instrumentation added** in this commit: `terminusFetchTask` now logs its own peak stack usage before `vTaskDelete`. There was previously no `uxTaskGetStackHighWaterMark` anywhere in the codebase, so no measured figure existed for any task. **Not yet read**: it only fires on a real fetch, which needs WiFi, and WiFi has not connected in this session.
- **Process note**: the agent that produced this report described the high-water log as already present at `Registration.cpp:396`. It was not — that is the line added during this same commit, which the agent read because the tree was edited while it was working. Concurrent edits contaminate agent findings; brief them against a quiescent tree.
- **Status**: open — instrumented, sizing decision blocked on one measurement; un-nesting proposed and not attempted

## 2026-08-29T10:15Z — WebDAV is compiled out on Arduino-ESP32 2.0.17; its RequestHandler overrides would not match anyway
- **Found by**: occupancy work (hunt-3 P0 `addHandler` diet), prompted by "replace 51 `server->on()` with one RequestHandler"
- **Where**: `src/network/server/WebDAVHandler.cpp:1` (`#if __has_include(<NetworkUdp.h>)`), `src/network/server/CrossPointWebServer.cpp` (`CROSSPOINT_HAS_NETWORKUDP`), framework `libraries/WebServer/src/detail/RequestHandler.h` (2-arg `canHandle(HTTPMethod, String)`)
- **What**: this platform is espressif32 6.12.0 / Arduino-ESP32 2.0.17. There is no `NetworkUdp.h` (that header is Arduino 3 / IDF 5). `CROSSPOINT_HAS_NETWORKUDP` is 0, so WebDAV is not linked. Hunt-3 cited WebDAV as the existing `addHandler` proof; the proof is the API, not a live handler on this device.
- **Why this matters**: copying `WebDAVHandler`'s 3-arg `canHandle(WebServer&, …) override` onto the dispatch table would **not override** the 2-arg virtual this core actually has. Matching must follow `RequestHandler.h` in the installed package, not the WebDAV TU.
- **Status**: open — WebDAV on Arduino 3 is a later port; not attempted

## 2026-08-29T10:16Z — Dispatch-table diet + Always start budget: Home+server is ~29–36 KB free, not ~11.5 KB; BWS no longer flaps
- **Found by**: occupancy implementation after hunt-3 P0, device walk on `192.168.86.51`
- **Where**: `WebRouteTableHandler`, `BackgroundServerPolicy::SERVER_STARTUP_BYTES`, `evaluateRunningHeap`, `CrossPointWebServer::begin(Background)`
- **What**: replacing ~84 `server->on()` handlers with one `RequestHandler` + flash table, deferring mDNS/WS until UDP `"hello"`, and dropping the 11,832 B route line from `SERVER_STARTUP_BYTES` (16,336 → 4,504) changed who holds memory. First flash after P0 but **before** the start-budget update: Home idle 37,348 free / 15,860 largest, TCP `ECONNREFUSED` on port 80 — Always's gate was still 40,912. After the budget update: UDP `crosspoint (on esp32c3-389228);81`, 35/35 HTTP 200 on `/api/status`, wifi stayed `Connected`, `freeHeap` 29,392–36,640 (HEAPPROF largest 17,396). `/api/settings` GET 200 (67 entries) at ~36 KB free. `POST /api/open-book` 202; reader page-turn via `CMD:BTN:PAGEFWD` rendered; AA still skipped (`OOM: grayscale strip scratch (8000 bytes)`). No `Retry scheduled (low heap)` / WiFi disconnect oscillation.
- **Why this matters**: the previous Always-mode picture (11.5 KB free / 8.2 KB largest, CSS skipped, settings 48 KB floor, server tearing down at the 16 KB running gate) was occupancy plus a start gate that still charged for the deleted handlers. Lowering every other threshold would not have produced this idle.
- **Status**: closed for P0 + start-budget + BWS KeepWifi abort; AA / font prewarm / Settings UI followed in 2026-08-29T13:50Z

## 2026-08-29T13:50Z — Page-turn AA no longer allocates; Settings UI streams one tab
- **Found by**: occupancy follow-on (deslop + remaining user-visible heap tells)
- **Where**: `ReaderUtils::renderAntiAliased`, `EpubReaderActivity::renderContents`, `SettingsActivity::rebuildSettingsLists`, `minHeapForFontPrewarm`
- **What**: the 8 KB grayscale strip scratch (and the 48 KB `storeBwBuffer` fallback) were a second buffer the 9–17 KB largest run could not reliably hold on top of the critical floor. AA now paints LSB/MSB into the live framebuffer, streams those planes, then **re-paints BW** and `cleanupGrayscaleWithFrameBuffer()` — the XTC pattern. Font prewarm gate 40,000 → 24,576 so a healthy post-diet reader (~29–39 KB) prewarms. On-device Settings no longer materializes `getSettingsList()` plus six category copies behind a 48 KB gate; it snapshots key/values, then keeps only the current tab. Terminus SD write was already un-nested (commit `94bf0a4`); the 12 KB fetch task remains until a high-water mark is read. Covers/carousel are still a 48 KB clone and cannot coexist with Always at 29–36 KB free.
- **Why this matters**: after occupancy, the remaining “cheap device” tells were luxuries that still *asked the heap for a second framebuffer-shaped object*. Who holds the pixels (the existing 48 KB panel buffer) was the fix, not a lower floor hoping 8 KB appears.
- **Status**: implemented; host tests gate the contracts. First device walk on occupancy firmware (2026-08-29T12:31Z): Home 42,152 free / 23,540 largest; UDP advertised `;81`; HTTP 6/6 Connected; no `OOM: grayscale strip scratch`. Settings tab walk aborted (CONFIRM ACK timeout). Re-walk after the 14:45Z overlay/open/shelf flash.

## 2026-08-29T14:45Z — Reader Options no longer reboots; destroy Home before book load; shelf before server
- **Found by**: ranked occupancy follow-on wave
- **Where**: `ReaderOptionsMemoryPolicy`, `ReaderOptionsActivity`, `ControlsOptionsActivity`, `ActivityManager::goToReader`/`completeOpenReader`, `BackgroundWifiService::refreshLibraryShelf`, `CrossPointWebServer::ensureWs`, `SettingsList::forEachSetting` category filter, `SettingsActivity::rebuildSettingsLists`
- **What**: the 96 KB / 48 KB overlay gate was a reboot-as-heap-remedy against a measured reader of ~42 KB / ~15 KB. Overlays now stream the same category-filtered `forEachSetting` as Settings and drop the 48 KB preview when `canRetainPreview` fails. `goToReader` now exits the current activity (and stack) on the next `ActivityManager::loop` tick, then stops both servers (`keepWifi`), reclaims SD-rebuildable caches, and only then calls `ReaderRegistry::open` — so the 32 KB inflate reservation sees Home gone. Library shelf refresh moved into the pre-server window. UDP `"hello"` advertises `;wsPort` only if `ensureWs()` admitted and constructed the socket. Settings rebuild stops both servers first, enumerates one category (Connect = System, System = System+Time), and does not construct rejected-category `SettingInfo`s.
- **Why this matters**: the remaining user-visible heap tell after occupancy was the Reader/Controls overlay reboot, not another threshold. Destroy-before-load is occupancy sequencing, not a new floor.
- **Not started**: STORED-shadow EPUB. Failed inflate still fails the load; we do not continue-and-log.
- **Status**: flashed and device-walked 2026-08-29 for overlays / destroy-before-load / Settings streaming. Shelf 84 KB skip, STORED-shadow, and PAGEFWD indexing reboot superseded by 2026-08-30T00:00Z.

## 2026-08-30T00:00Z — Post-index Continue, luxury thumbs/shelf, STORED-shadow first slice
- **Found by**: occupancy follow-on after 2026-08-29T14:45Z
- **Where**: `ReaderHeapRecoveryPolicy.h`, `EpubReaderActivity::loop`, `CoverThumbHeapPolicy.h`, `LibraryShelfRefreshPolicy.h`, `BackgroundWifiService`, `lib/ZipFile/StoredShadow.*`, `Epub::load`
- **What**: PAGEFWD after a dirty `createSectionFile` no longer calls `persistAndRestart`. Policy: Continue if largest ≥ 14 KB (measured successful post-index 14,836 / 16,372); else reclaim caches → covers → HTTP keep-Wi-Fi, at most twice, then continue. `Epub::load` returns false if the inflate window cannot be reserved. Cover thumbs skip unless the 32 KB window is already reserved or there is room for the window plus the 53,248 JPEG working set; a skip does not pin a failed ring. Library shelf admits at `HttpDownloader::MIN_HEAP_FOR_HTTPS` (38 KB), not 84 KB; luxury skip does not latch while `stopRequested`. STORED-shadow writes a method-0 ZIP beside cache (`stored.epub`, cap 16 entries) with a host round-trip; it is not wired into `Epub::load`. Original EPUB remains source of truth.
- **Why this matters**: reboot and Wi-Fi disconnect are not legal heap remedies. Indexing already wrote the section cache; a silent restart was destroying that work. Thumbs and the OPDS shelf are luxuries that must not steal the inflate run or HTTPS while Home is tearing down.
- **Status**: indexing Continue device-walked (Helm EPUB, cache clear, 6× PAGEFWD: one `enter EpubReader`, `Post-index Continue` action=0, no `persistAndRestartForRecovery`). Shelf at Always idle still cannot HTTPS (~22–31 KB vs 38 KB) — honest skip. STORED-shadow host-tested, not on the load path. Image-heavy chapters still cannot hold PNGdec (~58 KB) plus 16 KB headroom against ~16 KB largest; that path logs and does not reboot. 2026-08-30 walk: Helm FM page 0 is `img_1_0.png` 464×117 at (8,14) with `SELECTION_INDEX_FALLBACK empty` — the header-box / white page is that banner plus placeholder, not a dropped-text control-flow bug. PAGEFWD persist fail is TOC spine 2 empty-section refuse after PTX OOM (1 block dropped, 0 elements); no cheap skip-image fix. 2026-08-30T20:15Z: empty-section refuse now skips when the chapter attempted images (white page is the skipped PNG). Text-only chapters that truncate to zero elements still refuse. PNGdec itself is still occupancy-only.

## 2026-08-30T12:35Z — Terminus full-fetch high-water measured; Always path stops allocating a second task
- **Found by**: live device fetch after the Terminus server recovered
- **Where**: `src/features/terminus_sleep/Registration.cpp`, `BackgroundWifiService::run`, `/crosspoint-debug.log`
- **What**: multiple successful manifest -> image -> SD -> settings fetches used 2,244-2,256 bytes of the 12,288-byte `TerminusFetch` stack; the latest complete path logged 2,248 used / 10,040 free. The prior HTTP-500 manifest-only control used 2,168 bytes. With the SD write already un-nested, the full image path adds only ~88 bytes of stack depth. The fallback task is therefore reduced to 8,192 bytes with 5,936 bytes of observed margin. More importantly, the default Always-mode pre-server hook now executes the fetch synchronously on bgwifi's existing 8,192-byte stack, removing the transient task allocation and its allocator hole. The on-charge main-loop path retains the fallback task so network latency cannot freeze input.
- **Why this matters**: resizing alone would leave an 8 KB hole in the same allocation phase. Reusing bgwifi's resident stack removes that allocation from the path that precedes the EPUB inflate reservation, while preserving the asynchronous behavior required by the interactive on-charge dispatcher.
- **Status**: implemented; 570 host cases / 11,815 assertions pass in normal and fixed shuffled orders, and the locked `default` ESP32-C3 build succeeds (39.2% RAM, 91.9% flash). The host wrapper still exits on unrelated pre-existing configurator-schema drift. The Terminus translation unit compiles in the native simulator, whose full build is independently blocked later by the pinned simulator package's stale `CrossPointWebServer::begin()` signature. HTTP Always-path device acceptance is in the 2026-08-30T13:15Z entry. HTTPS remains a separate worst-case capture.

## 2026-08-30T13:15Z — Terminus Always fetch accepted on bgwifi's 8 KB stack
- **Found by**: flash of `firmware-20260830-1304-48ad43b-dirty.bin` to `192.168.86.51` (both OTA slots) and a boot-time Always poll against `http://192.168.86.25:2300`
- **Where**: `src/features/terminus_sleep/Registration.cpp` `runFetchOnCurrentTask("bgwifi pre-server")`, `/crosspoint-debug.log`, `GET /api/terminus/status`
- **What**: after flash, version `1.4.1-dev+3225`, uptime 39 s, `last_stage=ok`, `last_fetch_ok=true`. Durable log: manifest 415 B → image 5,883 B → `/sleep/trmnl_latest.png` pinned, then `bgwifi pre-server stack: 2560 of 8192 bytes used, 5632 free (69% margin)`. That is 304 B above the dedicated-task peak (2,256 B) for `taskEntry`/`run` frames, and 3,584 B above the 2,048 B free-stack floor. Immediately after the fetch, `begin()` saw 24,896 free / **9,204 largest** — below the fallback task preflight of 8,192+1,024=9,216 — so shrinking the dedicated task without reusing bgwifi would still have deferred this poll.
- **Why this matters**: the hole is gone on the default path, and the extra Always-path frames still leave 5,632 B free. The compile-time peak is raised to 2,560 B so the assert tracks the stack that actually runs the fetch.
- **Status**: HTTP Always-path accepted on device `.51`. On-charge fallback accepted in the 2026-08-30T13:40Z entry. HTTPS still uncaptured.

## 2026-08-30T13:40Z — Terminus on-charge fallback task accepted at 8 KB
- **Found by**: live switch of `backgroundServerMode` Always → Only on Charge on `192.168.86.51`, then `POST /api/terminus/test`, then restore Always
- **Where**: `src/features/terminus_sleep/Registration.cpp` `startFetchTask` / `runFetchOnCurrentTask("Fetch task")`, `BackgroundWebServer::startServer`, `/crosspoint-debug.log`
- **What**: on-charge recycled its own server (`Refresh due; recycling on-charge web server`), spawned the 8,192-byte `TerminusFetch` task, and completed manifest → 5,919-byte image → pin. Log: `Fetch task stack: 2352 of 8192 bytes used, 5840 free (71% margin)`. `BWS` deferred (`background network work in flight`) until the task finished, then `begin()` at 84,404 free / 57,332 largest. Always restored (`backgroundServerMode=2`). Local BYOS `http://192.168.86.25:2300` still has no TLS listener (HTTPS probe: wrong version on :2300, nothing on :443/:8443), and `POST /api/terminus/save` needs the API key, so the cloud/CRT-bundle stack remains unmeasured.
- **Why this matters**: the fallback is the path that must not freeze the on-charge main loop. 2,352 B used on a fresh 8 KB task leaves 5,840 B free, above the 2,048 B floor, and 208 B below the Always bgwifi peak (2,560 B) because those extra `taskEntry`/`run` frames are absent. The 8 KB hole is transient: it is gone before `begin()`.
- **Status**: HTTP on-charge fallback accepted. HTTPS still blocked on this LAN box.

## 2026-08-30T12:02Z — HttpDownloader HTTPS has no TLS verification option, so OPDS/shelf fail before connect
- **Found by**: grok — ad hoc device feature walk on `192.168.86.51` (`1.4.1-dev+3225`)
- **Where**: `src/network/http/HttpDownloader.cpp` `esp_http_client_config_t` at the fetch/probe/download sites (~179, ~281, ~350)
- **What**: Grimmory `https://arrakis-media.tailc1d084.ts.net:10000/api/v1/opds` and the Always-mode library-shelf refresh both log `ESP_ERR_HTTP_CONNECT` even at 67–69 KB free / 40–47 KB largest. The IDF line immediately before is `esp-tls-mbedtls: No server verification option set in esp_tls_cfg_t` → `ESP_ERR_MBEDTLS_SSL_SETUP_FAILED`. The TU comment still describes `setInsecure()` / no crt-bundle, but none of the three configs set `crt_bundle_attach` (OTA and Terminus do) or any insecure-skip flag, so mbedtls refuses to create the SSL handle. On-device UI: Grimmory "Failed to fetch feed". Serial: `Library shelf refresh failed` on every bgwifi start after the walk reboot.
- **Why not fixed here**: the walk was error collection, not an HTTP-client change. Terminus on this device is plain HTTP to `192.168.86.25:2300` and still succeeds.
- **Status**: device-proved 2026-08-30T14:12Z on `192.168.86.51` (flash md5 `207f6af81e62269272337cf843069023`, version still `1.4.1-dev+3225`, buildTimestamp 1788086246). Serial has no `No server verification option set` / `ESP_ERR_MBEDTLS_SSL_SETUP_FAILED`. Grimmory GET at 65 KB free completed TLS then failed `ESP_ERR_HTTP_MAX_REDIRECT (status 401)` — UI still "Failed to fetch feed", no catalog. Cold-boot shelf also logged `esp-x509-crt-bundle: PK verify failed with error FFFFBD78` / `mbedtls_ssl_handshake returned -0x3000`. Remaining failures are HTTP 401/redirect and occasional PK verify, not missing `crt_bundle_attach`. Full walk 2026-08-30T15:00Z (`runs/feature-walk-20260830-full`): still no missing-verification line. First Grimmory GET (73 KB free / 36 KB largest) failed `ESP_ERR_HTTP_CONNECT` / `sock < 0` with no mbedtls setup log; second GET after OPDS-exit reboot failed `mbedtls_ssl_setup returned -0x7F00` then CONNECT. No 401 this session. 2026-08-30T20:15Z: `sock < 0` at 64 KB was radio contention (Always HTTP + Terminus + 10× 401 retries), not the 38 KB `admitHttps` floor. OPDS now stops BG_WIFI before GET; 401 is one request; shelf fetch is quiet on luxury fail. Not flashed.

## 2026-08-30T12:02Z — Opening OPDS after the Helm EPUB silently restarts (34.8 KB < 38 KB)
- **Found by**: grok — same walk
- **Where**: `src/activities/browser/OpdsBookBrowserActivity.cpp:263` `silentRestart()`
- **What**: Continue-Reading into the Helm EPUB left Home at 33,452 free / 9,204 largest. Confirm on the Library/OPDS catalog tile then logged `Heap too low for HTTPS feed (34796 < 38000); silent restart`. `rst:0x3 (RTC_SW_SYS_RST)`. Occupancy already said reboot is not a legal heap remedy for indexing; this path still uses it for an HTTPS catalog the user explicitly opened. After reboot, the same Grimmory URL failed for the TLS-config bug above, not heap.
- **Why not fixed here**: out of scope for the walk. Related occupancy note: 2026-08-30T00:00Z (shelf skip at Always idle vs 38 KB).
- **Status**: device-proved 2026-08-30T14:13Z on the same flash. After Helm (home 32,116 free / 15,348 largest), Library enter logged `Heap too low for TLS (25368 < 38000, largest: 9204)` and showed Failed to fetch feed. No enter-path `Heap too low for HTTPS feed ... silent restart`. Exit still `OpdsBookBrowserActivity::onExit` → `recoverHeapAfterWifi("OPDS")` → `rst:0x3` under Always; that call was not this fix. Full walk 2026-08-30T15:00Z: Library BACK and Opds BACK each logged `Silent restart (target=0)` + `rst:0x3`. USB/IP stayed attached. No enter-path silent restart. 2026-08-30T20:15Z: `recoverHeapAfterWifi` no longer reboots (Always leaves WiFi up; otherwise tears the radio down and returns). OPDS enter also `BG_WIFI.stop(keepWifi)` before the catalog GET.

## 2026-08-30T12:02Z — Settings list draw clips at logical x=480
- **Found by**: grok — same walk
- **Where**: first occurrence logged from Settings DOWN into the More-tab list; `GFX` `Out-of-bounds pixel suppressed ... logical=(480,126) physical=(126,-1) panel=800x480 orientation=0`
- **What**: portrait width is 480, so x=480 is one past the last column. Rotation maps it to physical y=-1. Subsequent violations are silenced. Tab chrome still rendered; this is a clip, not a panic.
- **Why not fixed here**: walk-only.
- **Status**: device-proved gone 2026-08-30T14:16Z. Settings → More → DOWN onto Developer Mode: no `logical=(480,126)`. Screenshot `runs/feature-walk-20260830-retest/016-settings-more-list.png`. Same inclusive-x=480 class still fires in Planner (2026-08-30T14:18Z). Full walk 2026-08-30T15:00Z screenshot `runs/feature-walk-20260830-full/036-settings-more-list.png` (Developer Mode ON): still no `logical=(480,126)`.

## 2026-08-30T14:18Z — Grimmory OPDS now handshakes TLS, then dies on HTTP 401 / max redirects
- **Found by**: grok — retest walk after flashing the crt-bundle HttpDownloader fix to `192.168.86.51`
- **Where**: `src/network/http/HttpDownloader.cpp` `espFetch` / IDF `esp_http_client`; Grimmory `https://arrakis-media.tailc1d084.ts.net:10000/api/v1/opds`
- **What**: with `crt_bundle_attach` set, a healthy-heap Library open (72 KB free / 45 KB largest, activate id=3) no longer fails SSL setup. IDF then logs `HTTP_CLIENT: Error, reach max_redirection_count count=10` and firmware `ESP_ERR_HTTP_MAX_REDIRECT (status 401)`. UI: Grimmory "Failed to fetch feed". The stored OPDS server likely has no basic-auth header, or the 401 `Location` is a redirect loop the client follows to the cap. Distinct from the fixed missing-verification bug.
- **Why not fixed here**: this retest was proof of the three walk fixes, not OPDS auth/redirect policy.
- **Status**: open — catalog still empty on device; handshake is no longer the blocker. Full walk 2026-08-30T15:00Z did not reach 401: first catalog GET CONNECT at 64 KB free; post-reboot GET `ssl_setup -0x7F00`. UI still "Failed to fetch feed". Auth/redirect untested this session. 2026-08-30T20:15Z: firmware no longer treats 401 as 10 IDF authorization retries (`max_authorization_retries = -1`); one GET, then `STR_OPDS_AUTH_REQUIRED` from that status (no second `probeUrl`). Grimmory catalog still needs working OPDS username/password on the device — not invented here.

## 2026-08-30T14:18Z — Planner day-index header line clips at logical x=480
- **Found by**: grok — same retest walk, Extras → Planner
- **Where**: `src/activities/todo/DayIndexActivity.cpp:213` `drawLine(0, HEADER_HEIGHT, renderer.getScreenWidth(), HEADER_HEIGHT)` with `HEADER_HEIGHT = 50`. Sibling: `DayDetailActivity.cpp:522` uses `getScreenWidth()` the same way at y=69.
- **What**: inclusive `drawLine` x2 is portrait width 480, one past the last column. Serial: `GFX Out-of-bounds pixel suppressed ... logical=(480,50) physical=(50,-1)`. Same class as the Settings More clip (2026-08-30T12:02Z), different call site. UI still showed "Daily Planner" / "A fresh page".
- **Why not fixed here**: out of scope for the Settings-tab underline fix; this walk was proof, not a second clip sweep.
- **Status**: open — full walk 2026-08-30T15:00Z confirmed `logical=(480,50)` on Planner enter; UI "Daily Planner" / "A fresh page" (`023-planner.png`). 2026-08-30T20:15Z: Planner `drawLine` x2 clamped to `getScreenWidth()-1` (DayIndex, DayDetail, FontSelection). Host-tested; not flashed.

## 2026-08-30T20:15Z — recoverHeapAfterWifi Always branch was a reboot, not a heap check
- **Found by**: grok — full-walk error fix (`runs/feature-walk-20260830-full`)
- **Where**: `src/main.cpp` `recoverHeapAfterWifi`; callers OPDS / FONT / KOAUTH
- **What**: under Always the function disconnected STA and called `silentRestart()` with no heap measurement. The 90 KB "skip reboot" threshold on the non-Always path was also above every measured live heap, so any WiFi activity exit rebooted. Occupancy policy (2026-08-30T00:00Z): reboot is not a legal heap remedy.
- **Why not fixed here**: it is fixed in this session. Always now leaves WiFi up; otherwise the radio is torn down and the activity returns. Settings already proved Always can restart without `rst:0x3`.
- **Status**: implemented, host-tested; not flashed.

## 2026-08-30T20:15Z — Settings still stops Always for the enumeration heap budget
- **Found by**: grok — same full-walk fix pass
- **Where**: `SettingsActivity::rebuildSettingsLists` `BG_WIFI.stop(true)` + `BackgroundWebServer::stop(true)`
- **What**: opening Settings tears down the background HTTP server (~20 s ECONNREFUSED on Terminus/file-transfer until reconcile restarts Always). That is expected, not a crash. Keeping the server resident while `forEachSetting` enumerates was not shown to fit the Settings heap budget; the stop is the existing occupancy sequencing (2026-08-29T14:45Z).
- **Why not fixed here**: not cheap and not proven safe. A 20 s gap with automatic return is left as designed.
- **Status**: wontfix (heap budget). Not a crash.

## 2026-08-30T20:15Z — Grimmory empty catalog after TLS is 401, not missing CRT bundle
- **Found by**: grok — same pass; retest `ESP_ERR_HTTP_MAX_REDIRECT (status 401)` at 65–72 KB free
- **Where**: `HttpDownloader::espFetch`; Grimmory `https://arrakis-media.tailc1d084.ts.net:10000/api/v1/opds`
- **What**: ESP-IDF counts HTTP 401 authorization retries against `max_redirection_count` (default 10). Firmware now sets `max_authorization_retries = -1` and surfaces 401/403 as `STR_OPDS_AUTH_REQUIRED` from the first GET. KOReader credentials file was absent this walk (`No credentials file found`). Catalog fill still requires a working OPDS username/password on the device; none were invented.
