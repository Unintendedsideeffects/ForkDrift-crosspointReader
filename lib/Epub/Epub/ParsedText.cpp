    ...
    for (size_t j = i; j < totalWordCount; ++j) {
      ...
      if (j + 1 < totalWordCount && continuesVec[j + 1]) {
        ...
        if (!noSpaceBeforeVec[j + 1]) {
          nextGap = renderer.getKerning(fontId, lastCodepoint(words[j]), firstCodepoint(words[j + 1]), wordStyles[j]);
        }
  ...
  if (willReorder) {
    ...
    for (size_t wordIdx = 0; wordIdx < reorderedWidthsScratch.size(); wordIdx++) {
      ...
      if (nextIsContinuation) {
        ...
        // wordIdx > 0 mirrors the gap-counting loop above (which skips index 0): a leading
        // no-break space must not receive justifyExtra (upstream #2185/#2298).
