...
AutomatonState decodeState(const EmbeddedAutomaton& automaton, size_t addr) {
  ...
  if (hasLevels) {
    ...
    // offset < 4 would underflow `offset - 4u` (size_t) to a huge value, pointing
    // levelsPtr before the blob. Validate both bounds.
    if (offset < 4u || offset + levelsLen > automaton.size) {
      return AutomatonState{};
    }
