...
bool CssParser::saveToCache(HalFile& file) const {
  if (!file) {
    return false;
  }
...
bool CssParser::saveToCache() const {
  ...
  HalFile file;
  if (!Storage.openFileForWrite("CSS", cacheDir_ + rulesCache, file)) {
    return false;
  }
...
bool CssParser::loadFromCache(HalFile& file) {
  if (!file) {
    return false;
  }
  ...
  // Read each simple rule: selector string + CssStyle fields
  ...
  if (file.read(&descendantCount, sizeof(descendantCount)) != sizeof(descendantCount)) {
    LOG_DBG("CSS", "Truncated CSS cache: missing descendant rule count");
    rulesBySelector_.clear();
    return false;
  }

  if (descendantCount > MAX_DESCENDANT_RULES) {
    LOG_DBG("CSS", "Invalid descendant rule count in cache (%u > %zu)", descendantCount, MAX_DESCENDANT_RULES);
    rulesBySelector_.clear();
    return false;
  }
  ...
  for (uint16_t i = 0; i < descendantCount; ++i) {
    uint16_t ancLen = 0;
    if (file.read(&ancLen, sizeof(ancLen)) != sizeof(ancLen) || ancLen == 0 || ancLen > MAX_SELECTOR_LENGTH) {
      LOG_DBG("CSS", "Invalid ancestor selector length in descendant cache");
      rulesBySelector_.clear();
      descendantRules_.clear();
      return false;
    }
    std::string ancestorSelector;
    ancestorSelector.resize(ancLen);
    if (file.read(&ancestorSelector[0], ancLen) != ancLen) {
      rulesBySelector_.clear();
      descendantRules_.clear();
      return false;
    }
    ...
    uint16_t subLen = 0;
    if (file.read(&subLen, sizeof(subLen)) != sizeof(subLen) || subLen == 0 || subLen > MAX_SELECTOR_LENGTH) {
      LOG_DBG("CSS", "Invalid subject selector length in descendant cache");
      rulesBySelector_.clear();
      descendantRules_.clear();
      return false;
    }
    std::string subjectSelector;
    subjectSelector.resize(subLen);
    if (file.read(&subjectSelector[0], subLen) != subLen) {
      rulesBySelector_.clear();
      descendantRules_.clear();
      return false;
    }
    ...
    CssStyle style;
    if (!readStyle(style)) {
      LOG_DBG("CSS", "Truncated CSS cache in descendant rule style");
      rulesBySelector_.clear();
      descendantRules_.clear();
      return false;
    }

    descendantRules_.push_back({std::move(ancestorSelector), std::move(subjectSelector), style});
  }
...
bool CssParser::loadFromCache() {
  ...
  HalFile file;
  if (!Storage.openFileForRead("CSS", cacheDir_ + rulesCache, file)) {
    return false;
  }
