...
static bool isProgressiveJpeg(HalFile& file) {
  file.seek(0);
  uint8_t buf[2];
...
bool JpegToBmpConverter::jpegFileToBmpStreamInternal(HalFile& jpegFile, Print& bmpOut, int targetWidth,
  ...
  const bool smoothUpscale = progressive && needsScaling && effectiveSrcW <= outWidth && effectiveSrcH <= outHeight;
  ...
  if (smoothUpscale) {
    ...
    if (!heapguard::canAllocate(smoothRowsBytes, heapguard::kCriticalFloorBytes)) {
      LOG_ERR("JPG", "Skipping progressive smoothing: low heap (%zu bytes)", smoothRowsBytes);
      return false;
    }
