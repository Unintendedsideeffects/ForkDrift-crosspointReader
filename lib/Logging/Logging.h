...
#ifndef SIMULATOR
    // Serial CMD: lines can be large (base64 OTA_DATA chunks, ~2KB; see the
    // CMD: handler and handleSerialOtaCommand in main.cpp); HWCDC's default RX
    // buffer (256B) drops inbound bytes whenever the main loop can't drain
    // promptly — an e-ink render blocks it for seconds. 8KB holds one
    // outstanding line with headroom. Must be called before begin() to take
    // effect. Only costs
