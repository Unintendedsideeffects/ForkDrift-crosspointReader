...
void SdFirmwareUpdateActivity::performUpdate() {
  ...
  if (result != firmware_flash::Result::OK) {
    ...
    // BAD_CHIP here is the re-validation catching a wrong-MCU image the
    // pre-confirmation pass missed — e.g. the SD card was swapped in between.
    errorMessage =
...
void SdFirmwareUpdateActivity::render(RenderLock&&) {
  ...
  if (state == State::VALIDATING) {
    ...
    const auto hintLines = renderer.wrappedText(UI_10_FONT_ID, tr(STR_RESTARTING_HINT), hintWidth, 3);
    int y = hintY;
