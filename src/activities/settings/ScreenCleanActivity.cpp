...
void ScreenCleanActivity::render(RenderLock&&) {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, tr(STR_SCREEN_CLEAN));
  ...
  GUI.drawList(
      ...
      [](const int index) { return std::string(subtitleForAction(index)); }, [](const int) { return UIIcon::Image; });
