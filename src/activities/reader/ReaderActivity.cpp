...
std::unique_ptr<Epub> ReaderActivity::loadEpub(const std::string& path) {
  ...
  auto epub = std::unique_ptr<Epub>(new (std::nothrow) Epub(path, "/.crosspoint"));
  if (!epub) {
...
std::unique_ptr<Xtc> ReaderActivity::loadXtc(const std::string& path) {
  ...
  auto xtc = std::unique_ptr<Xtc>(new (std::nothrow) Xtc(path, "/.crosspoint"));
  if (!xtc) {
...
std::unique_ptr<Txt> ReaderActivity::loadTxt(const std::string& path) {
  ...
  auto txt = std::unique_ptr<Txt>(new (std::nothrow) Txt(path, "/.crosspoint"));
  if (!txt) {
