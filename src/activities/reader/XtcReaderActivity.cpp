...
void XtcReaderActivity::loop() {
  ...
  const bool skipPages = SETTINGS.longPressChapterSkip && mappedInput.getHeldTime() > ReaderUtils::SKIP_HOLD_MS;
...
void XtcReaderActivity::renderPage() {
  ...
  ScopedBuffer pageBuffer(pageBufferSize);
  if (!pageBuffer) {
  ...
  if (bitDepth == 2) {
    ...
    for (uint16_t srcY = 0; srcY < maxSrcY; srcY++) {
      ...
      for (uint16_t srcX = 0; srcX < pageWidth; srcX++) {
        ...
        const bool isBlack = !((pageBuffer.data()[srcByte] >> srcBit) & 1);  // XTC: 0 = black, 1 = white
