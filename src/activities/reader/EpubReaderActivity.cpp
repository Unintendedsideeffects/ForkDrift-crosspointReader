...
void EpubReaderActivity::onEnter() {
  ...
  if (APP_STATE.pendingBookmarkSpine != PENDING_BOOKMARK_SPINE_NONE && APP_STATE.pendingBookmarkProgress >= 0.0f) {
    ...
    APP_STATE.pendingBookmarkProgress = PENDING_BOOKMARK_PROGRESS_NONE;
  }
  ...
  switch (action) {
    ...
    case EpubReaderMenuActivity::MenuAction::BOOKMARK_TOGGLE: {
      ...
      if (BOOKMARKS.hasBookmarkForPage(spineIdx, pageProgress, section->pageCount)) {
        ...
        if (tocIdx >= 0) {
          snprintf(chapterTitle, sizeof(chapterTitle), "%s", epub->getTocItem(tocIdx).title.c_str());
        }
        const auto addResult = BOOKMARKS.addBookmark(spineIdx, pageProgress, section->pageCount, chapterTitle);
        if (addResult == BookmarkStore::AddResult::LimitReached) {
    ...
    case EpubReaderMenuActivity::MenuAction::VIEW_HIGHLIGHTS: {
      startActivityForResult(std::make_unique<EpubReaderHighlightListActivity>(renderer, mappedInput),
                             [this](const ActivityResult& result) {
                               if (!result.isCancelled) {
                                 const auto& highlight = std::get<ProgressChangeResult>(result.data);
                                 RenderLock lock(*this);
...
void EpubReaderActivity::render(RenderLock&& lock) {
  ...
  if (selectionMode) {
    ...
    if (selectionNeedsWordReload) {
      if (!section || !refreshSelectionWords()) {
        exitSelectionMode();
        return;
      }
      selectionBaseSnapshot.reset();
...
#if ENABLE_BOOKMARKS
  const bool isPageBookmarked = BOOKMARKS.hasBookmarkForPage(
...
bool EpubReaderActivity::refreshSelectionWords() {
  if (!section) {
    return false;
  }
  auto page = section->loadPageFromSectionFile();
...
void EpubReaderActivity::selectionTurnPage(const bool forward) {
  if (!selectionMode || !section) {
    return;
  }
  const int spineBefore = currentSpineIndex;
  pageTurn(forward);
  if (currentSpineIndex != spineBefore || !section) {
    exitSelectionMode();
    return;
  }
  selModel.words.clear();
...
std::string EpubReaderActivity::selectedText() const {
  ...
  return selection::joinSpanFormatted(selModel.words, lo, hi);
}
  ...
  for (const selection_capture::Action action : actions) {
    switch (action) {
      ...
      case selection_capture::Action::RemoveHighlight:
        ...
        break;
#endif
      case selection_capture::Action::Close:
        items.push_back(I18N.get(StrId::STR_SELECTION_CLOSE));
        break;
      default:
        break;
    }
  ...
  selectionPopup.show(StrId::STR_SELECT_TEXT, items, initialIndex, [this, actions](int idx) {
    ...
    switch (actions[static_cast<size_t>(idx)]) {
      ...
      case selection_capture::Action::Anki: {
        ...
        selectionPreferredAction = selection_capture::Action::Anki;
      ...
      case selection_capture::Action::BookNotes: {
        ...
        if (ok) {
          selectionPreferredAction = selection_capture::preferredAfterNotesSuccess();
        } else {
...
bool EpubReaderActivity::handleSelectionInput() {
  ...
  if (mappedInput.wasPressed(MappedInputManager::Button::PageBack)) {
    selectionTurnPage(false);
    return true;
  }
  if (mappedInput.wasPressed(MappedInputManager::Button::PageForward)) {
    selectionTurnPage(true);
    return true;
  }
