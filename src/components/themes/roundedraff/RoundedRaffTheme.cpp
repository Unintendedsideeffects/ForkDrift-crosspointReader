...
void RoundedRaffTheme::drawHeader(const GfxRenderer& renderer, Rect rect, const char* title,
  ...
  const bool showBatteryPercentage =
      showHeaderBattery && SETTINGS.hideBatteryPercentage != CrossPointSettings::HIDE_BATTERY_PERCENTAGE::HIDE_ALWAYS;
  ...
  if (showHeaderBattery && showBatteryPercentage) {
    const int maxTextWidth = renderer.getTextWidth(SMALL_FONT_ID, "100%");
    batteryGroupLeftX -= maxTextWidth + batteryPercentSpacing;

    // Clear a fixed-width area to avoid ghosting when the percentage digit count changes.
