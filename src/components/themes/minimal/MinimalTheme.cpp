...
// Local copy of LyraTheme.cpp's icon resolver (24px subset) — the upstream
// helper is file-local to LyraTheme.cpp, so MinimalTheme replicates the
// compact-file-browser icon set it needs.
const uint8_t* minimalIconForName(UIIcon icon) {
  switch (icon) {
    case UIIcon::Folder:
      return Folder24Icon;
    case UIIcon::Text:
...
int minimalCompactRowHeight(const GfxRenderer& renderer) {
...
void MinimalTheme::drawList(const GfxRenderer& renderer, Rect rect, int itemCount, int selectedIndex,
  ...
  const int fileRowHeight = minimalCompactRowHeight(renderer);
  ...
  for (int i = pageStartIndex; i < itemCount && i < pageEndIndex; i++) {
    ...
    const uint8_t* iconBitmap = minimalIconForName(rowIcon(i));
...
void MinimalTheme::drawButtonHints(GfxRenderer& renderer, const char* btn1, const char* btn2, const char* btn3,
  ...
  const int buttonY = MinimalMetrics::values.buttonHintsHeight;
