...
bool BookmarkStore::loadForBook(const std::string& filePath, const std::string& title, const std::string& author,
  ...
  const uint32_t crc = static_cast<uint32_t>(std::hash<std::string>{}(filePath));
...
bool BookmarkStore::removeBookmarkAt(size_t index) {
  ...
  bookmarks.erase(bookmarks.begin() + static_cast<std::ptrdiff_t>(index));
  dirty = true;
  saveToFile();
  return true;
}

bool BookmarkStore::hasBookmarkForPage(uint16_t spineIndex, float pageProgress, int pageCount) const {
...
bool BookmarkStore::getAllBookmarkedBooks(std::vector<BookmarkedBookEntry>& out) {
  ...
  for (const auto& name : files) {
    ...
    if (!readCheckedString(title) || !readCheckedString(author) || !readCheckedString(path)) continue;
