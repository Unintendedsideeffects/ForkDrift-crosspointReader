...
static int l_gui_draw_rect(lua_State* L) {
  ...
  r->drawRect(luaL_checkinteger(L, 1), luaL_checkinteger(L, 2), luaL_checkinteger(L, 3), luaL_checkinteger(L, 4),
              color != Color::White);
  return 0;
}
...
static int l_gui_fill_rect(lua_State* L) {
  ...
  if (!r) return 0;
  Color color = get_lua_color(L, 5);
  r->fillRect(luaL_checkinteger(L, 1), luaL_checkinteger(L, 2), luaL_checkinteger(L, 3), luaL_checkinteger(L, 4),
              color != Color::White);
  return 0;
}
...
static int l_gui_draw_line(lua_State* L) {
  ...
  r->drawLine(luaL_checkinteger(L, 1), luaL_checkinteger(L, 2), luaL_checkinteger(L, 3), luaL_checkinteger(L, 4), lw,
              color != Color::White);
  return 0;
}
...
static int l_gui_draw_rounded_rect(lua_State* L) {
  ...
  r->drawRoundedRect(luaL_checkinteger(L, 1), luaL_checkinteger(L, 2), luaL_checkinteger(L, 3), luaL_checkinteger(L, 4),
                     lw, radius, color != Color::White);
  return 0;
}
...
static int l_gui_fill_rounded_rect(lua_State* L) {
  ...
  Color color = get_lua_color(L, 6);
  r->fillRoundedRect(luaL_checkinteger(L, 1), luaL_checkinteger(L, 2), luaL_checkinteger(L, 3), luaL_checkinteger(L, 4),
                     radius, color);
  return 0;
}
...
namespace {
void localDrawCircle(GfxRenderer* r, int xc, int yc, int rad, int lw, bool black) {
  for (int y = -rad; y <= rad; y++) {
    for (int x = -rad; x <= rad; x++) {
      if (x * x + y * y >= (rad - lw) * (rad - lw) && x * x + y * y <= rad * rad) {
        r->drawPixel(xc + x, yc + y, black);
      }
    }
  }
}
void localFillCircle(GfxRenderer* r, int xc, int yc, int rad, bool black) {
  for (int y = -rad; y <= rad; y++) {
    for (int x = -rad; x <= rad; x++) {
      if (x * x + y * y <= rad * rad) {
        r->drawPixel(xc + x, yc + y, black);
...
static int l_gui_draw_circle(lua_State* L) {
  ...
  Color color = get_lua_color(L, 5);
  localDrawCircle(r, x, y, rad, lw, color != Color::White);
  return 0;
}
...
static int l_gui_fill_circle(lua_State* L) {
  ...
  localFillCircle(r, x, y, rad, color != Color::White);
  return 0;
}
...
static int l_gui_draw_text(lua_State* L) {
  ...
  r->drawText(luaL_checkinteger(L, 1), luaL_checkinteger(L, 2), luaL_checkinteger(L, 3), luaL_checkstring(L, 4), color,
...
static int l_gui_draw_centered_text(lua_State* L) {
  ...
  r->drawCenteredText(luaL_checkinteger(L, 1), luaL_checkinteger(L, 2), luaL_checkstring(L, 3), color,
                      (EpdFontFamily::Style)style);
  return 0;
}
...
static int l_gui_draw_button_hints(lua_State* L) {
  ...
  auto* m = get_input(L);
  if (!r) return 0;
  const char* back = luaL_optstring(L, 1, "Back");
  const char* confirm = luaL_optstring(L, 2, "Select");
  const char* prev = luaL_optstring(L, 3, "Up");
  const char* next = luaL_optstring(L, 4, "Down");
  if (m) {
...
static int l_gui_set_orientation(lua_State* L) {
  ...
  if (strcmp(mode, "landscape_cw") == 0)
    o = GfxRenderer::LandscapeClockwise;
  else if (strcmp(mode, "landscape_ccw") == 0)
    o = GfxRenderer::LandscapeCounterClockwise;
  else if (strcmp(mode, "portrait_inv") == 0)
    o = GfxRenderer::PortraitInverted;
  r->setOrientation(o);
  return 0;
}
...
static int l_input_is_any_pressed(lua_State* L) {
  ...
  bool any = false;
  if (m) {
    any = m->isPressed(MappedInputManager::Button::Confirm) || m->isPressed(MappedInputManager::Button::Back) ||
          m->isPressed(MappedInputManager::Button::Left) || m->isPressed(MappedInputManager::Button::Right) ||
          m->isPressed(MappedInputManager::Button::Up) || m->isPressed(MappedInputManager::Button::Down);
  }
...
static int l_net_wifi_status(lua_State* L) {
  ...
  if (s_wifiState == NetWifiState::CONNECTING)
    str = "connecting";
  else if (s_wifiState == NetWifiState::CONNECTED)
    str = "connected";
  else if (s_wifiState == NetWifiState::FAILED)
    str = "failed";
...
static int l_net_get(lua_State* L) {
  ...
  if (!headers.empty()) {
    ...
    if (std::string(url).find("https://") == 0) {
...
bool LuaManager::begin(GfxRenderer* r, MappedInputManager* input) {
  ...
  L = luaL_newstate();
  if (!L) {
    LOG_ERR("LUA", "OOM: Cannot create state");
    return false;
  }
...
void LuaManager::registerBindings() {
  ...
  // gui.*
  lua_newtable(L);
  lua_pushcfunction(L, l_gui_clear);
  lua_setfield(L, -2, "clear");
  lua_pushcfunction(L, l_gui_refresh);
  lua_setfield(L, -2, "refresh");
  lua_pushcfunction(L, l_gui_draw_rect);
  lua_setfield(L, -2, "drawRect");
  lua_pushcfunction(L, l_gui_fill_rect);
  lua_setfield(L, -2, "fillRect");
  lua_pushcfunction(L, l_gui_draw_line);
  lua_setfield(L, -2, "drawLine");
  lua_pushcfunction(L, l_gui_draw_rounded_rect);
  lua_setfield(L, -2, "drawRoundedRect");
  lua_pushcfunction(L, l_gui_fill_rounded_rect);
  lua_setfield(L, -2, "fillRoundedRect");
  lua_pushcfunction(L, l_gui_draw_text);
  lua_setfield(L, -2, "drawText");
  lua_pushcfunction(L, l_gui_draw_centered_text);
  lua_setfield(L, -2, "drawCenteredText");
  lua_pushcfunction(L, l_gui_get_text_width);
  lua_setfield(L, -2, "getTextWidth");
  lua_pushcfunction(L, l_gui_get_width);
  lua_setfield(L, -2, "width");
  lua_pushcfunction(L, l_gui_get_height);
  lua_setfield(L, -2, "height");
  lua_pushcfunction(L, l_gui_set_orientation);
  lua_setfield(L, -2, "setOrientation");
  lua_pushcfunction(L, l_gui_draw_bmp);
  lua_setfield(L, -2, "drawBmp");
  lua_pushcfunction(L, l_gui_draw_button_hints);
  lua_setfield(L, -2, "drawButtonHints");
  lua_pushcfunction(L, l_gui_draw_pixel);
  lua_setfield(L, -2, "drawPixel");
  lua_pushcfunction(L, l_gui_draw_circle);
  lua_setfield(L, -2, "drawCircle");
  lua_pushcfunction(L, l_gui_fill_circle);
  lua_setfield(L, -2, "fillCircle");
  lua_pushcfunction(L, l_gui_fill_polygon);
  lua_setfield(L, -2, "fillPolygon");

  // Hint constants
  lua_pushstring(L, "Back");
  lua_setfield(L, -2, "HINT_BACK");
  lua_pushstring(L, "OK");
  lua_setfield(L, -2, "HINT_OK");
  lua_pushstring(L, "Prev");
  lua_setfield(L, -2, "HINT_PREV");
  lua_pushstring(L, "Next");
  lua_setfield(L, -2, "HINT_NEXT");
  lua_pushstring(L, "Up");
  lua_setfield(L, -2, "HINT_UP");
  lua_pushstring(L, "Down");
  lua_setfield(L, -2, "HINT_DOWN");

  lua_setglobal(L, "gui");

  // input.*
  lua_newtable(L);
  lua_pushcfunction(L, l_input_was_pressed);
  lua_setfield(L, -2, "wasPressed");
  lua_pushcfunction(L, l_input_was_released);
  lua_setfield(L, -2, "wasReleased");
  lua_pushcfunction(L, l_input_is_pressed);
  lua_setfield(L, -2, "isPressed");
  lua_pushcfunction(L, l_input_is_any_pressed);
  ...
  lua_pushcfunction(L, l_sys_delay);
  ...
  // fs.*
  lua_newtable(L);
  lua_pushcfunction(L, l_fs_list_dirs);
  lua_setfield(L, -2, "listDirs");
  lua_pushcfunction(L, l_fs_list_files);
  lua_setfield(L, -2, "listFiles");
  lua_pushcfunction(L, l_fs_exists);
  lua_setfield(L, -2, "exists");
  lua_pushcfunction(L, l_fs_read_file);
  lua_setfield(L, -2, "readFile");
  lua_pushcfunction(L, l_fs_write_file);
  lua_setfield(L, -2, "writeFile");
  lua_setglobal(L, "fs");

  // net.*
  lua_newtable(L);
  lua_pushcfunction(L, l_net_wifi_connect);
  lua_setfield(L, -2, "wifiConnect");
  lua_pushcfunction(L, l_net_wifi_status);
  lua_setfield(L, -2, "wifiStatus");
  lua_pushcfunction(L, l_net_wifi_disconnect);
  lua_setfield(L, -2, "wifiDisconnect");
  lua_pushcfunction(L, l_net_get);
  lua_setfield(L, -2, "get");
  lua_pushcfunction(L, l_net_urlencode);
  lua_setfield(L, -2, "urlencode");
  lua_setglobal(L, "net");

  // Refresh mode constants
  lua_pushinteger(L, HalDisplay::FULL_REFRESH);
  lua_setglobal(L, "REFRESH_FULL");
  lua_pushinteger(L, HalDisplay::HALF_REFRESH);
  lua_setglobal(L, "REFRESH_HALF");
  lua_pushinteger(L, HalDisplay::FAST_REFRESH);
  lua_setglobal(L, "REFRESH_FAST");

  // Color constants
  lua_pushinteger(L, (int)Color::Clear);
  lua_setglobal(L, "COLOR_CLEAR");
  lua_pushinteger(L, (int)Color::White);
  lua_setglobal(L, "COLOR_WHITE");
  lua_pushinteger(L, (int)Color::LightGray);
  lua_setglobal(L, "COLOR_LIGHT_GRAY");
  lua_pushinteger(L, (int)Color::DarkGray);
  lua_setglobal(L, "COLOR_DARK_GRAY");
  lua_pushinteger(L, (int)Color::Black);
  lua_setglobal(L, "COLOR_BLACK");

  // Font ID constants
  lua_pushinteger(L, NOTOSERIF_14_FONT_ID);
  lua_setglobal(L, "FONT_BOOKERLY_14");
  lua_pushinteger(L, NOTOSERIF_12_FONT_ID);
  lua_setglobal(L, "FONT_BOOKERLY_12");
  lua_pushinteger(L, NOTOSERIF_16_FONT_ID);
  lua_setglobal(L, "FONT_BOOKERLY_16");

  // Font style constants
  lua_pushinteger(L, (int)EpdFontFamily::REGULAR);
  lua_setglobal(L, "STYLE_REGULAR");
  lua_pushinteger(L, (int)EpdFontFamily::BOLD);
  lua_setglobal(L, "STYLE_BOLD");
  lua_pushinteger(L, NOTOSANS_12_FONT_ID);
  lua_setglobal(L, "FONT_NOTOSANS_12");
  lua_pushinteger(L, NOTOSANS_14_FONT_ID);
  lua_setglobal(L, "FONT_NOTOSANS_14");
  lua_pushinteger(L, NOTOSANS_16_FONT_ID);
  lua_setglobal(L, "FONT_NOTOSANS_16");
...
bool LuaManager::runPlugin(const std::string& pluginName) {
  ...
  if (!reader.file) {
    LOG_ERR("LUA", "File missing: %s", path.c_str());
    return false;
  }
