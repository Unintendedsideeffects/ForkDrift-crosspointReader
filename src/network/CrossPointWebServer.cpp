  ...
  for (const auto& s : settings) {
    ...
    switch (s.type) {
      ...
      case SettingType::ENUM: {
        ...
        const size_t optionCount = !s.enumStringValues.empty()
                                       ? s.enumStringValues.size()
                                       : (s.dynamicValuesGetter ? s.dynamicValuesGetter().size() : s.enumValues.size());
