...
* `platformio.local.ini`: Local overrides (gitignored; sets `build_dir` under `~/.cache/crosspoint-pio-build` by default)
...
* **Environments** (in `platformio.ini`):
  ...
  * `custom`: Feature profile from `platformio-custom.ini` (generate with `scripts/generate_build_config.py`)
  * `simulator`: Desktop SDL2 build (see platformio.ini comments)
...
1. **HTML Headers** (generated by `scripts/build_html.py`):
   ...
   - **Source**: HTML templates in `src/network/html/*.html`
